...
public class CustomViewAbove extends ViewGroup {
	...
	private AboveTouchInterceptor mAboveTouchInterceptor;
	...
	public void setAboveTouchInterceptor(AboveTouchInterceptor interceptor) {
		mAboveTouchInterceptor = interceptor;
	}
	...
	private boolean thisTouchAllowed(MotionEvent ev) {
		if (isMenuOpen()) {
			...
			if (mAboveTouchInterceptor != null) {
				int x = (int) ev.getX();
				int y = (int) ev.getY();
				if (mAboveTouchInterceptor.getRect().contains(x, y)) {
					return false;
				}
			}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			mActivePointerId = ev.getAction()
					& ((Build.VERSION.SDK_INT >= 8) ? 65280 : 65280);
